...
async function handleApi(req, res, url) {
  ...
  const missingPredictionsMatch = url.pathname.match(/^\/api\/players\/([^/]+)\/missing-predictions$/);
  if (missingPredictionsMatch && method === "POST") {
    const payload = await readBody(req);
    const db = await readDb();
    const player = db.players.find((item) => item.id === missingPredictionsMatch[1]);
    if (!player) {
    ...
    const existingGameIds = new Set(
      db.predictions
        .filter((prediction) => prediction.playerId === player.id)
        .map((prediction) => prediction.gameId)
    );
    const missingGames = db.games.filter((game) => !existingGameIds.has(game.id));
    if (missingGames.length === 0) {
      sendJson(res, 200, { ok: true, player: publicPlayerEntry(db, player), standings: buildStandings(db) });
      return;
    }
    ...
    for (const game of missingGames) {
      if (game.status && game.status !== "upcoming") {
        sendError(res, 400, `O jogo ${game.homeTeam} x ${game.awayTeam} ja comecou ou terminou. Fale com o administrador.`);
        return;
      }
      const prediction = predictionsByGame.get(game.id);
      const homeScore = prediction ? normalizeScore(prediction.homeScore) : null;
